...
- [x] Inbound (contribution) reconciliation matching — Nomba's real field confirmed against a live sandbox call: inbound `vact_transfer` rows report `virtualAccountReference` (never `merchantTxRef`), echoing back the `accountRef` originally passed to `createVirtualAccount()`, which is `contributions.virtualAccountRef` locally. `nomba.client.ts` now has a `correlatingRef(tx)` accessor (`merchantTxRef` for outbound, `virtualAccountReference` for inbound), and `reconciliation.service.ts` joins through `contributions` to resolve inbound refs back to local `transactions` rows. Also fixed while here: `fetchTransactions` was calling the parent-account-scoped `GET /v1/transactions/accounts`, which never returned sub-account funding rows at all — switched to `POST /v1/transactions/accounts/{subAccountId}`. Regression tests in `reconciliation.test.ts`.
