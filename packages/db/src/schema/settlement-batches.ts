 ...
 * what our ledger expected for the period (expectedAmountKobo, summed from
 * our own transactions) against what the provider's settlement report says
 * actually moved (reportedAmountKobo). status tracks the outcome of that
 * comparison; reconciliationRecords holds the line-item detail behind it.
 */
...
export const settlementBatches = pgTable('settlement_batches', {
  ...
  reportedAmountKobo: bigint('reported_amount_kobo', { mode: 'bigint' }),
