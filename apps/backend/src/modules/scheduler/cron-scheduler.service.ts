...
const schedules: ScheduleDef[] = [
  ...
  // remaining sweeps run in a predictable sequence rather than colliding.
...
// Schedulers previously registered by an id no longer in `schedules` above — upsertJobScheduler
// only adds/updates, it never prunes a removed entry's already-registered repeatable job out of
// Redis on its own, so registerAll() explicitly removes these once on every boot. Safe to call
// even if nothing was ever registered under this id (removeJobScheduler just returns false).
const removedScheduleIds = ['reconciliation-frequent'];

export const CronSchedulerService = {
  async registerAll() {
    for (const id of removedScheduleIds) {
      await payoutCronQueue.removeJobScheduler(id);
    }
