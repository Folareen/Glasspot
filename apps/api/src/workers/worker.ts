...
import db, { pots, targetBasedPayoutConfigs, contributionPayments } from '@glasspot/db';
...
import { recurringPayoutConfigs, rotationPayoutLegs } from '@glasspot/db';
 ...
 * for recurring_payout_configs and rotation_payout_configs.
 */
async function applyOnSuccess(onSuccess: DisbursementOnSuccess) {
  switch (onSuccess.type) {
    ...
    case 'mark_rotation_leg_fired':
      await db
        .update(rotationPayoutLegs)
        .set({ fired: true, firedAt: new Date() })
        .where(eq(rotationPayoutLegs.id, onSuccess.rotationLegId));
      return;
  }
}
